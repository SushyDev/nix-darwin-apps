...
aarch64() {
	...
	read -r current_version current_sha256 < <(get_current_package_info "aarch64-darwin" "dbeaver")

	local -r page_url='https://formulae.brew.sh/api/cask/dbeaver-community.json'
	...
	local -r download_url="https://dbeaver.io/files/${new_version}/dbeaver-ce-${new_version}-macos-aarch64.dmg"
...
x86_64() {
	...
	read -r current_version current_sha256 < <(get_current_package_info "x86_64-darwin" "dbeaver")
	...
	local -r page_content="$(fetch_page_content $page_url)"
	local -r new_version="$(extract_version_from_url "$page_content" $version_pattern)"
	local -r download_url="https://dbeaver.io/files/${new_version}/dbeaver-ce-${new_version}-macos-x86_64.dmg"
	local -r new_sha256="$(fetch_sha256_from_url "$download_url")"
...
main() {
	log_info "Starting DBeaver update check"
