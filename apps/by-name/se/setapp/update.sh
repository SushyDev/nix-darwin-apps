...
extract_url_from_content() {
	...
	log_info "Extracting URL from content"

	local -r url="$(echo "$content" | jq -r $pattern)"

	[ -n "$url" ] && [ "$url" != "null" ] || die "Failed to extract URL from content"
...
universal() {
	...
	read -r current_version current_sha256 < <(get_current_package_info "aarch64-darwin" "setapp")

	local -r page_url='https://formulae.brew.sh/api/cask/setapp.json'
	local -r version_pattern='.version'
	local -r url_pattern='.url'
	...
	local -r download_url="$(extract_url_from_content "$page_content" $url_pattern)"
...
main() {
	log_info "Starting Setapp update check"
