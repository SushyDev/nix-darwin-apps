...
universal() {
	...
	read -r current_version current_sha256 < <(get_current_package_info "aarch64-darwin" "slack")

	local -r page_url='https://formulae.brew.sh/api/cask/slack.json'
	...
	local -r download_url="https://downloads.slack-edge.com/desktop-releases/mac/universal/${new_version}/Slack-${new_version}-macOS.dmg"
...
main() {
	log_info "Starting Slack update check"
